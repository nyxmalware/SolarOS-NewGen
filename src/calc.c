...
vbe_draw_string(win->x+8,win->y+70,"Type: + - * /",COL_DKGRAY,COL_LTGRAY);
}
static void calc_on_key(char ch){
if(ch=='\n'){
int64_t a=0,b=0;
char op=0;
int p=0;
...
case '-':calc_result=a-b;break;
case '*':calc_result=a*b;break;
case '/':if(b!=0)calc_result=a/b;else calc_result=0;break;
...
calc_has_result=0;
calc_result=0;
} else if(calc_pos<62&&ch>=0x20){
calc_expr[calc_pos++]=ch;
calc_expr[calc_pos]=0;
}
}
void calc_window_create(void){
window_t *win=window_create(200,100,240,120,"Calculator");
if(win){
